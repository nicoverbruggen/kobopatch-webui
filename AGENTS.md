...
- The NickelMenu `.adds/nm/items` file is generated, not a static asset. A feature contributes entries via `menuItems(ctx)`, returning `{ id, order, lines }` objects; `installer.js` collects them from every selected feature, sorts by `order`, and renders the file. Device-conditional items are a simple "don't include this entry" (e.g. custom-menu drops Dark Mode on unsupported hardware). The base Tweak menu and its tab header are owned by the `custom-menu` feature. Features that inject `experimental:` NickelMenu config lines do so in `postProcess`, which runs after the items file is assembled.
- Every on-device "toggle + reboot" Tweak item (and the `.adds/nm/scripts/*.sh` it runs) is owned by the feature it toggles — no capability flags, no `custom-menu` coordination, no name-based special-casing. `simplify-tabs` contributes its "Toggle navigation tabs" entry and ships `toggle_tabs.sh`. The three home-screen hiders are generated from one table in `features/hide-home-content/` (each appends a distinct `hide_home_*_enabled` flag in `postProcess`) and all contribute the *same* shared "Show/hide home content" entry plus the *same* universal `toggle_hidden_home.sh` (which flips every `hide_home_*_enabled` flag at once). To make several features safely contribute one shared toggle, `installer.js` de-duplicates `menuItems` entries by `id` and install files by `path` (keeping the first), so the shared item and script appear exactly once however many hiders are selected. A feature whose `ctx.asset()` (scoped to `features/<id>/`) can't reach a shared asset may `fetch()` it by its real path, as the hiders and KOReader do.
