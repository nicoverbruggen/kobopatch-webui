...
- Keep a feature's behavior and logic colocated in its `features/<feature>/` module. A feature owns everything specific to it — generated files (`install`), output adjustments such as device-conditional changes (`postProcess`), declarative `Kobo eReader.conf` settings (`confSettings`), its user-facing copy (`reviewNotices`), and metadata such as `title`, `description`, `section`, and an optional `hint` (a URL or plain text; renders a "?" badge in the feature list that opens the link in a new tab, or shows the text in a popup). The flow and installer must stay generic: they invoke these feature hooks and render/apply whatever a feature declares, rather than special-casing an individual feature by name.
