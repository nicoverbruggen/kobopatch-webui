...
- The NickelMenu `.adds/nm/items` file is generated, not a static asset. A feature contributes entries via `menuItems(ctx)`, returning `{ id, order, lines }` objects; `installer.js` collects them from every selected feature, sorts by `order`, and renders the file. Device-conditional items are a simple "don't include this entry" (e.g. custom-menu drops Dark Mode on unsupported hardware). The base Tweak menu and its tab header are owned by the `custom-menu` feature. Features that inject `experimental:` NickelMenu config lines (the hide-* flags, simplify-tabs) still do so in `postProcess`, which runs after the items file is assembled. A feature can declare the capability flag `hidesHomeContent` or `hidesTabs` so `custom-menu` adds a universal "toggle + reboot" Tweak item (and ships its `.adds/nm/scripts/*.sh` script) when any such feature is installed — keeping the flow/installer generic (flag-based, not name-based).
