...
When the entire process has finished (files copied), validate that every file was written correctly.

If a file does not come up as verified, try to copy it again. If that, fails tell the user that the installation may not work as expected, and that they should try the manual installation option.
