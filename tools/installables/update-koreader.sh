...
#
# Usage: tools/installables/update-koreader.sh
...
if [ -f "$DIST_DIR/koreader-release.json" ]; then
    CURRENT=$(jq -r '.version' "$DIST_DIR/koreader-release.json")
...
echo "{\"version\":\"$VERSION\"}" > "$DIST_DIR/koreader-release.json"
