...
export default {
    ...
    async install(ctx) {
        ...
        ctx.progress(label);
        const zipBytes = await fetchInstallableAsset('koreader', 'koreader-kobo.zip', {
            onProgress: downloadProgress(ctx.progress, label, await installableSize('koreader')),
            errorPrefix: 'Failed to download KOReader',
        });
