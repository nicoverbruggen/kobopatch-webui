...
if grep -q "^Screenshots=true" "$CONFIG_FILE"; then
    ...
    echo ""
    echo "Important: the button cannot lock or wake your Kobo in this mode. Remember to select Screenshots again when you're done."
elif grep -q "^Screenshots=false" "$CONFIG_FILE"; then
    echo "Screenshots are now disabled!"
    echo ""
    echo "Your power button works normally again and can be used to lock or wake up your device."
else
    echo "The screenshot setting was toggled, but its new state could not be read from Kobo eReader.conf."
fi
