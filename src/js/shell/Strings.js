/**
 * Strings.js — every user-facing string in one place.
 *
 * The export is `TL`, not `STRINGS`, and the file is not named after it. That
 * mismatch is deliberate: `TL.js` would be a two-letter filename that tells a
 * reader nothing, and renaming the export reaches hundreds of call sites for no
 * gain at the point of use — `TL.ERROR.PATCH_FAILED` and
 * `STRINGS.ERROR.PATCH_FAILED` read the same. Please do not "fix" it.
 */
