...
test.describe('NickelMenu — install', () => {
    ...
    test('with device — previous manifest selections are labelled without changing current choices', async ({ page }) => {
        await connectMockDevice(page, {
            extraAddsFiles: [{ path: ['nm', 'webui-preset'], content: '# Generated by KoboPatch Web UI\n' }],
            extraRootFiles: [
                {
                    path: ['.kobopatch-webui', 'nickelmenu.json'],
                    content: JSON.stringify({ selected: ['simplify-tabs', 'hide-notices'] }),
                },
            ],
        });
        ...
        await expect(simplifyTabs.locator('.nm-config-previous')).toHaveText('You selected this last time');
        await expect(hideNotices.locator('.nm-config-previous')).toHaveText('You selected this last time');
    ...
    test('with device — reinstalling with KOReader over a prior install regenerates the menu file (no duplicate entries)', async ({ page }) => {
        ...
        // The prior install is detected, so the preset card invites a reinstall.
        await expect(page.locator('#nm-option-preset-title')).toHaveText('(Re)install with preset (and customize)');
