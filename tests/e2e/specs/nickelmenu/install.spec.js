...
test.describe('NickelMenu — install', () => {
    ...
    test('with device — a modify run that fails after removing reports it and keeps the audit log', async ({ page }) => {
        test.skip(!hasNickelMenuAssets(), 'NickelMenu assets not found in webroot');

        // The worst realistic outcome of a modify: the deletions land, then the
        // write fails. The user must be told, and the log must say what went.
        await connectMockDevice(page, {
            failWritePaths: ['KOBOeReader/.kobo/KoboRoot.tgz'],
        ...
        // The failure is surfaced as a partial application, not a silent stop.
        await expect(page.locator('#step-error')).not.toBeHidden();
        await expect(page.locator('#error-message')).toContainText('partly applied');
        await expect(page.locator('#btn-error-download-log')).toBeVisible();

        // The removal happened before the failure and is not rolled back.
        expect(await mockPathExists(page, '.kobo', 'screensaver', 'moon.png')).toBe(false);

        // The log is not written to the device here — that write would likely fail
        // too — so the offered download is the user's only record of what went.
        const [download] = await Promise.all([page.waitForEvent('download'), page.click('#btn-error-download-log')]);
        const log = fs.readFileSync(await download.path(), 'utf8');
        expect(log, 'the downloadable log must name what was removed').toContain('Screensaver');
        expect(log).toContain('.kobo/screensaver/moon.png');
    });
    ...
    test('with device — deselecting an installed feature deletes its files from the device', async ({ page }) => {
        await connectMockDevice(page, {
            extraAddsFiles: [{ path: ['nm', 'webui-preset'], content: '# Generated by KoboPatch Web UI\n' }],
            extraRootFiles: [
                {
        ...
        // The whole point of the modify flow: unticking something installed takes
        // it off the device, during a run that is otherwise an install.
        expect(await mockPathExists(page, '.kobo', 'screensaver', 'moon.png'), 'the deselected screensaver should be gone').toBe(false);
        // And the run still installed, rather than only removing.
        expect(await mockPathExists(page, '.adds', 'nm', 'webui-preset')).toBe(true);
    });

    test("with device — dropping a font family removes only that family's files", async ({ page }) => {
        test.skip(!hasFontAssets(), 'Font assets not found (run npm run setup:installables)');
            ...
            extraRootFiles: [
                {
                    ...
                    content: JSON.stringify({
                        selected: ['custom-menu', 'additional-fonts'],
                        features: {},
                        configuration: { fonts: { families: ['readerly', 'libron'] } },
                        meta: { writer: { version: '1.60' } },
                    }),
                },
                { path: ['fonts', 'KF_Readerly-Regular.ttf'], content: 'font' },
                { path: ['fonts', 'KF_Libron-Regular.ttf'], content: 'font' },
        ...
        // Drop one of the two recovered families through the fonts dialog.
        await page.click('#nm-fonts-summary');
        await expect(page.locator('#nm-fonts-dialog')).toBeVisible();
        await page.uncheck('#nm-fonts-core-list input[data-family-id="libron"], #nm-fonts-extra-list input[data-family-id="libron"]');
        ...
        // additional-fonts' reconcile hook clears the dropped family only.
        expect(await mockPathExists(page, 'fonts', 'KF_Libron-Regular.ttf'), 'the dropped family should be gone').toBe(false);
        expect(await mockPathExists(page, 'fonts', 'KF_Readerly-Regular.ttf'), 'the kept family should survive').toBe(true);
    });

    // A whole journey rather than one mechanism: someone who set this Kobo up a
    // while ago with a renamed Toggle menu and renamed tabs comes back to drop a
    // feature, checks their work by stepping back, and installs. It is the case
    // most likely to be hit in practice, and it crosses recovery, preselection,
    // back-navigation and the write.
    test('with device — returning to an existing setup keeps its customizations while dropping a feature', async ({ page }) => {
        const preset = [
            '# Generated by KoboPatch Web UI',
            'experimental :menu_main_15505_label :Tweaks',
        ...
        await connectMockDevice(page, {
            extraAddsFiles: [{ path: ['nm', 'webui-preset'], content: preset }],
            extraRootFiles: [
                {
                    path: ['.kobopatch-webui', 'nickelmenu.json'],
                    content: JSON.stringify({ selected: ['custom-menu', 'simplify-tabs', 'screensaver'] }),
                },
                { path: ['.kobo', 'screensaver', 'moon.png'], content: 'sample' },
            ],
        });
        ...
        // The card offers to modify, not to reinstall from scratch.
        await expect(page.locator('#nm-option-preset-title')).toHaveText('Modify current setup (and customize)');
        await page.click('input[name="nm-option"][value="preset"]');
        await page.click('#btn-nm-next');

        // Their own menu name came back off the device, not a default.
        await expect(page.locator('#step-nm-features')).not.toBeHidden();
        await expect(page.locator('#nm-custom-menu-summary .nm-config-summary-label')).toHaveText('Tweaks');
        await expect(page.locator('input[name="nm-cfg-simplify-tabs"]')).toBeChecked();

        // Drop the screensaver.
        await openNmSection(page, 'Legacy');
        await expect(page.locator('input[name="nm-cfg-screensaver"]')).toBeChecked();
        await page.uncheck('input[name="nm-cfg-screensaver"]');

        // Step back to double-check, then forward again. The choice must survive:
        // re-entering the step must not re-apply the device's state over it.
        await page.click('#btn-nm-features-back');
        await expect(page.locator('#step-nickelmenu')).not.toBeHidden();
        await page.click('#btn-nm-next');
        await expect(page.locator('#step-nm-features')).not.toBeHidden();
        await openNmSection(page, 'Legacy');
        await expect(page.locator('input[name="nm-cfg-screensaver"]'), 'the untick must survive back-navigation').not.toBeChecked();
        await expect(page.locator('#nm-custom-menu-summary .nm-config-summary-label')).toHaveText('Tweaks');
        ...
        // The dropped feature is gone from the device.
        expect(await mockPathExists(page, '.kobo', 'screensaver', 'moon.png'), 'the dropped screensaver should be gone').toBe(false);

        // And their customizations were written back rather than reset to defaults.
        const items = await readMockFile(page, '.adds', 'nm', 'webui-preset');
        expect(items, 'the custom menu name must survive a modify run').toContain('experimental :menu_main_15505_label :Tweaks');
        expect(items, 'renamed tabs must survive a modify run').toContain('experimental :menu_main_15505_1_label: Library');
        expect(items).toContain('experimental :menu_main_15505_2_label: Progress');
    });
