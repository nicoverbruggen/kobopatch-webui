...
test.describe('NickelMenu', () => {
  test('no device — install with config via manual download', async ({ page }) => {
    ...
    // Sideloaded Mode is the one feature with a hint, so exactly one "?" badge shows.
  ...
  test('with device — remove NickelMenu removes the typography toggle script and sync exclusions when nothing else remains', async ({ page }) => {
    ...
    await connectMockDevice(page, {
      ...
      extraAddsFiles: [
        { path: ['scripts', 'toggle_typography.sh'], content: '#!/bin/sh\ntoggle' },
      ],
    });
    ...
    await page.click('#btn-mode-next');
    await page.click('input[name="nm-option"][value="remove"]');
    // Detected via its toggle script and pre-checked for removal.
    ...
    expect(await mockPathExists(page, '.adds', 'scripts', 'toggle_typography.sh')).toBe(false);
  ...
  test('with device — remove NickelMenu keeps the typography toggle script and sync exclusions when its removal is unchecked', async ({ page }) => {
    test.skip(!hasNickelMenuAssets(), 'NickelMenu assets not found in webroot');
    ...
    await page.click('#btn-mode-next');
    await page.click('input[name="nm-option"][value="remove"]');
    // Keep better typography: its script stays, so .adds/scripts remains and the
    // sync exclusions that cover it must be kept too.
    ...
    expect(await mockPathExists(page, '.adds', 'scripts', 'toggle_typography.sh')).toBe(true);
  ...
  test('with device — remove NickelMenu keeps .adds/scripts and sync exclusions when user scripts remain', async ({ page }) => {
    ...
    // The feature's own script is removed, but an unrelated user script keeps
    // .adds/scripts alive, so the sync exclusions covering it stay too.
    expect(await mockPathExists(page, '.adds', 'scripts', 'toggle_typography.sh')).toBe(false);
  ...
  test('with device — factory-reset Kobo (not signed in) recommends Sideloaded Mode', async ({ page }) => {
    test.skip(!hasNickelMenuAssets(), 'NickelMenu assets not found in webroot');
    ...
    // auto-expanded so the Sideloaded Mode option is visible.
    await expect(page.locator('#nm-sideloaded-banner')).toBeVisible();
    await expect(page.locator('#nm-sideloaded-banner')).toContainText('Sideloaded Mode');
  ...
  test('with device — signed-in Kobo does not recommend Sideloaded Mode', async ({ page }) => {
    test.skip(!hasNickelMenuAssets(), 'NickelMenu assets not found in webroot');
  ...
  test('with device — enabling Sideloaded Mode writes SideloadedMode to the config', async ({ page }) => {
    test.skip(!hasNickelMenuAssets(), 'NickelMenu assets not found in webroot');
  ...
  test('with device — Sideloaded Mode is disabled on Kobo software older than 4.31', async ({ page }) => {
    test.skip(!hasNickelMenuAssets(), 'NickelMenu assets not found in webroot');

    // 4.28 is below Sideloaded Mode's 4.31 minimum (but still a supported OS).
