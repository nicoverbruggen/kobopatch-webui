...
test('featuresToInstall always includes required features, even when nothing is selected', () => {
    ...
    assert.ok(result.some(f => f.id === 'custom-menu'), 'required custom-menu is always installed');
});
...
test('featuresToInstall does not gate when firmware is unknown (manual mode)', () => {
    ...
    assert.ok(result.some(f => f.id === 'sideloaded-mode'), 'no firmware floor blindly applied');
});
...
test('optionalCleanupToRemove / optionalCleanupKept partition detected by the checked ids', () => {
    ...
    assert.deepEqual(optionalCleanupToRemove(sess, detected).map(f => f.id), ['a', 'c']);
    assert.deepEqual(optionalCleanupKept(sess, detected).map(f => f.id), ['b']);
});

test('featureReviewNotices flattens notices and forwards deviceInfo', () => {
    let seen = null;
    const features = [
        { reviewNotices: (ctx) => { seen = ctx.deviceInfo; return [{ title: 'one' }]; } },
        { /* no reviewNotices */ },
        { reviewNotices: () => [{ title: 'two' }] },
    ];
    const notices = featureReviewNotices(features, { firmware: '4.40.0' });
    assert.deepEqual(notices.map(n => n.title), ['one', 'two']);
...
test('nmReviewModel (remove) reports removed vs kept from the detected cleanups', () => {
    ...
    const model = nmReviewModel(
        session({ nickelMenuOption: 'remove', nmOptionalCleanupIds: ['a'] }),
        detected,
        { firmware: '4.40.0' },
    );
    assert.equal(model.mode, 'remove');
    assert.deepEqual(model.removedFeatures.map(f => f.id), ['a']);
    assert.deepEqual(model.keptFeatures.map(f => f.id), ['b']);
});

test('nmReviewModel (preset) lists the install features including required ones', () => {
    const model = nmReviewModel(
        session({ nickelMenuOption: 'preset', selectedFeatureIds: ['screensaver'] }),
        [],
        { firmware: '4.40.0' },
    );
...
test('nmReviewModel (nickelmenu-only) installs no preset features', () => {
    const model = nmReviewModel(
        session({ nickelMenuOption: 'nickelmenu-only', selectedFeatureIds: ['screensaver'] }),
        [],
        { firmware: '4.40.0' },
    );
